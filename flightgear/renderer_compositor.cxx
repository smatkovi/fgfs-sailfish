...
FGRenderer::update( ) {
    if (!_position_finalized || !_scenery_loaded->getBoolValue())
    {
        ...
        /* glGetIntegerv ohne aktuellen GL-Kontext ist undefiniert.
           update() laeuft in der Hauptschleife, nicht im Renderthread -
           Desktop-Treiber liefern hier Unsinn (siehe Kommentar unten),
           der Mali-Blob stuerzt ab. Die Abfrage wandert deshalb hinter
           eine Kontextpruefung. */
        GLint max_texture_size = 0;
        {
            osgViewer::Viewer* v = globals->get_renderer()->getViewer();
            osg::GraphicsContext* gc = nullptr;
            if (v && v->getCamera()) gc = v->getCamera()->getGraphicsContext();
            if (gc && gc->isRealized() && gc->makeCurrent()) {
