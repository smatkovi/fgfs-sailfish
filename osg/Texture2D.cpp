...
void Texture2D::apply(State& state) const
{
    ...
    else if (_image.valid() && _image->data())
    {
        ...
        else
        {
            ...
            for (int guard = 8; guard > 0 && glGetError() != 0; --guard) {}   /* FlightGear GLES port: clear stale errors */
#endif
               ...
               bypass the State error check; log the first ones with their result. */
            {
                ...
                {
                    ++logged;
                    if (logged <= 40) OSG_WARN << "Texture2D upload " << (image->getFileName().empty() ? std::string("(inline)") : image->getFileName())
                                              << " " << _textureWidth << "x" << _textureHeight
                                              << " pixfmt=0x" << std::hex << image->getPixelFormat() << " internal=0x" << _internalFormat
                                              << " type=0x" << image->getDataType() << std::dec << " levels=" << _numMipmapLevels
                                              << " mipmapImage=" << image->isMipmap() << " glGetError=0x" << std::hex << err << std::dec << std::endl;
                }
            }
#endif
        }
    ...
    if (textureObject != 0 && _texMipmapGenerationDirtyList[contextID])
    {
...
#if !defined(OSG_GL_FIXED_FUNCTION_AVAILABLE)
        {
            static int logged = 0;
            GLenum err = glGetError();
            if (err != 0 && logged < 12) { ++logged; OSG_WARN << "Texture2D generateMipmap glGetError=0x" << std::hex << err << std::dec << std::endl; }
        }
#endif
    }
}
